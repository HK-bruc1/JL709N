...
// puts 目前无法提取字符串常量
#define puts(format)  {if(config_dlog_enable){dlog_printf(0, "%s\n", (const char *)format);}else{puts(format);}}
...
#define printf(format, ...)  {if(config_dlog_enable){dlog_printf(0, format, ##__VA_ARGS__);}else{printf(format, ##__VA_ARGS__);}}
