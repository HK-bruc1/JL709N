    ...
    list_for_each_entry(node, &hdl->sync_list[ch->attr.ch_idx], entry) {
        ...
        if (node->network == AUDIO_NETWORK_LOCAL && ch->state == AUDIO_CFIFO_START) {
            if (audio_syncts_latch_enable(node->hdl)) {
                ...
                if (hdl->channel == 1) {
                    flag = 2;
                    buffered_frames = (alink_get_dma_len(hdl->hw_alink) / point_offset - swn / point_offset - 1) * flag - audio_cfifo_channel_unread_diff_samples(&ch->fifo);
                } else {
                    if (hdl->channel == 4) {
                        flag = 2;
                    }
...
static int audio_iis_channel_fifo_write(struct audio_iis_channel *ch, void *data, int len, u8 is_fixed_data)
{
    ...
    u8 flag = 1; //shn是4byte为单位，cififo是2byte为单位
    if (hdl->channel == 1) {
        flag = 2;
    } else if (hdl->channel == 4) {
        flag = 2;
    ...
    if (frames) {
        if (hdl->channel == 4) {
            audio_iis_set_shn(hdl, ch->attr.ch_idx, frames * point_offset * 4 * flag);
        } else {
            audio_iis_set_shn(hdl, ch->attr.ch_idx, frames * point_offset * 4 / flag);
        }
    }
    ...
    if (hdl->channel == 1) {
        ASSERT(swp == audio_cfifo_get_write_offset(&hdl->cfifo[ch->attr.ch_idx])*point_offset / flag, ", %d, %d %d, %d\n", shn, swp, audio_cfifo_get_write_offset(&hdl->cfifo[ch->attr.ch_idx])*point_offset, hdl->channel);
    } else {
        ASSERT(swp == audio_cfifo_get_write_offset(&hdl->cfifo[ch->attr.ch_idx])*point_offset * flag, ", %d, %d %d, %d\n", shn, swp, audio_cfifo_get_write_offset(&hdl->cfifo[ch->attr.ch_idx])*point_offset * flag, hdl->channel);
    }
