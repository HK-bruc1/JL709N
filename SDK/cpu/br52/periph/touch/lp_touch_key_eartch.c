...
static u8 eartch_last_state = CTMU_P2M_EARTCH_OUT_EVENT;
...
void lp_touch_key_eartch_parm_init(void)
{
    ...
    M2P_CTMU_EARTCH_CH = (__this->eartch_ref_ch << 4) | (__this->eartch_ch & 0xf);

    __this->eartch_trim_flag = 0;
    __this->eartch_inear_ok = 0;
    __this->eartch_trim_value = 10000;
    ...
    if (ret > 0) {
        ...
        log_debug("eartch_trim_value = %d", __this->eartch_trim_value);
    }
    M2P_CTMU_EARTCH_TRIM_VALUE_L = (__this->eartch_trim_value & 0xFF);
    M2P_CTMU_EARTCH_TRIM_VALUE_H = (__this->eartch_trim_value >> 8) & 0xFF;

    M2P_CTMU_INEAR_VALUE_L  = __this->eartch_soft_inear_val & 0xFF;
    M2P_CTMU_INEAR_VALUE_H  = __this->eartch_soft_inear_val >> 8;
    M2P_CTMU_OUTEAR_VALUE_L = __this->eartch_soft_outear_val & 0xFF;
    M2P_CTMU_OUTEAR_VALUE_H = __this->eartch_soft_outear_val >> 8;
}
...
static void lp_touch_key_eartch_event_handle(u8 eartch_state)
{
    ...
    if ((touch_bt_tool_enable) && (touch_bt_online_debug_key_event_handle)) {
        if (touch_bt_online_debug_key_event_handle(__this->eartch_ch, (void *)&e)) {
            return;
        }
    }
#endif
...
static void lp_touch_key_eartch_diff_trim(void)
{
    ...
    if (__this->eartch_trim_value == 0) {
        ...
        __this->eartch_trim_value = (eartch_diff + __this->eartch_trim_value) / 2;
    }
    log_debug("eartch ch trim value: %d, res = %d", __this->eartch_trim_value, eartch_diff);

    __this->eartch_trim_flag ++;

    if (__this->eartch_trim_flag > 20) {
        __this->eartch_trim_flag = 0;
        is_lpkey_active = 0;

        M2P_CTMU_CH_DEBUG &= ~BIT(__this->eartch_ch);
        int ret = syscfg_write(LP_KEY_EARTCH_TRIM_VALUE, &(__this->eartch_trim_value), sizeof(__this->eartch_trim_value));
        if (ret > 0) {
            M2P_CTMU_EARTCH_TRIM_VALUE_L = (__this->eartch_trim_value >> 0) & 0xff;
            M2P_CTMU_EARTCH_TRIM_VALUE_H = (__this->eartch_trim_value >> 8) & 0xff;
            state = EARTCH_STATE_TRIM_OK;
            __this->eartch_inear_ok = 1;
            log_debug("trim: %d\n", __this->eartch_inear_ok);
        } else {
            state = EARTCH_STATE_TRIM_ERR;
            __this->eartch_inear_ok = 0;
            log_debug("trim: %d\n", __this->eartch_inear_ok);
        }
...
static void lp_touch_key_eartch_event_deal(u8 ch)
{
    if (__this->eartch_en == 0) {
        return;
    }
    if (__this->eartch_ch != ch) {
        return;
    }
    ...
    if (P2M_CTMU_EARTCH_EVENT == eartch_last_state) {
        return;
    }

    eartch_last_state = P2M_CTMU_EARTCH_EVENT;
    lp_touch_key_eartch_event_handle(eartch_last_state);
}
...
static u8 lp_touch_key_testbox_remote_test(u8 ch, u8 event)
{
    if (__this->eartch_en == 0) {
        return 0;
    }
